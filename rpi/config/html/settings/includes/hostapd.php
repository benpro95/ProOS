...
function DisplayHostAPDConfig(){
  ...
  $arrConfig = array();
  $arrChannel = array('g');
  $arrSecurity = array( 2 => 'WPA2');
  $arrEncType = array('CCMP' => 'CCMP');
  exec("ip -o link show | awk -F': ' '{print $2}'", $interfaces);

    if (isset($_POST['ResetHostAPDSettings'])) {
      echo '<div class="alert alert-warning">Reboot System to Apply Changes.</div>';
      $result = shell_exec("touch /dev/null");
    }
  ...
  <div class="row">
    <div class="col-lg-12">
      <div class="panel panel-primary">
        <div class="panel-heading"><i class="fa fa-dot-circle-o fa-fw"></i> Hotspot Information</div>
        <!-- /.panel-heading -->
...
function SaveHostAPDConfig($wpa_array, $enc_types, $modes, $interfaces, $status) {
  ...
  if ($good_input) {
    if ($tmp_file = fopen('/tmp/hostapddata', 'w')) {
      ...
      system( "touch /dev/null " . RASPI_HOSTAPD_CONFIG, $return );
      if( $return == 0 ) {
