...
christmass_new_year() {
  date_d=$(date '+%d' | sed 's/0//g')
  date_m=$(date '+%m' | sed 's/0//g')
...
if ! [[ "${os_codename}" =~ (precise|maya|trusty|qiana|rebecca|rafaela|rosa|xenial|sarah|serena|sonya|sylvia|bionic|tara|tessa|tina|tricia|cosmic|disco|eoan|focal|groovy|jessie|stretch|continuum|buster|bullseye) ]]; then
  clear
  header_red
...
armhf_recommendation() {
  ...
  if [[ "${print_architecture}" == 'armhf' && "${is_cloudkey}" == "false" ]]; then
    ...
    if [[ "${os_codename}" =~ (precise|trusty|xenial|bionic|cosmic|disco|eoan|focal|groovy) ]]; then
...
if [[ "${os_codename}" =~ (disco|eoan|focal|groovy) && "${architecture}" =~ (amd64|arm64) ]]; then
  header
...
if ! dpkg -l | grep "^ii\\|^hi" | grep -iq "mongodb-server\\|mongodb-org-server"; then
  ...
  elif [[ "${os_codename}" =~ (bionic|tara|tessa|tina|tricia|disco|eoan|focal|groovy) && "${architecture}" == "i386" ]]; then
    ubuntu_32_mongo
    libssl_temp="$(mktemp --tmpdir=/tmp libssl1.0.2_XXXXX.deb)" || abort
  ...
  elif [[ "${os_codename}" =~ (xenial|bionic|cosmic|disco|eoan|focal|groovy|sarah|serena|sonya|sylvia|tara|tessa|tina|tricia) && "${architecture}" =~ (amd64|arm64) ]]; then
    if [[ "${mongo_version_supported}" == "3.6.999" ]]; then
      mongodb_36_key
    else
      mongodb_34_key
...
if [[ "${remote_controller}" == 'true' ]]; then
  if ! curl -sk "https://${PUBLIC_SERVER_IP}:${dash_port}/api/system" | grep -ioq "api.err.InvalidObject"; then remote_controller=false; fi
fi
